...
export interface Transition<TStatus extends string> {
  ...
  von: TStatus;
  nach: TStatus;
  /** Erlaubte Rollen. Leeres Array = nur SYSTEM (z.B. Cron). */
...
/** Für eine Rolle in einem Status erlaubte (nicht-System-)Aktionen. */
export function erlaubteAktionen<S extends string>(
  transitions: readonly Transition<S>[],
  von: S,
  rolle: Rolle,
): readonly Transition<S>[] {
  return transitions.filter((t) => !t.system && t.von === von && t.rollen.includes(rolle));
}
